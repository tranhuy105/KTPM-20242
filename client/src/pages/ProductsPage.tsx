...
const ProductsPage = () => {
    ...
    const [filters, setFilters] =
        useState<ProductFiltersType>(initialFilters);
    ...
    useEffect(() => {
        const fetchAvailableFilters = async () => {
            ...
            try {
                const data =
                    await productApi.getAllProductFilters();
                setAvailableFilters(data);
            } catch (error) {
                console.error(
                    "Error fetching available filters:",
                    error
                );
            } finally {
    ...
    useEffect(() => {
        ...
        if (filters.filters?.search)
            params.set("search", filters.filters.search);
        ...
        if (filters.filters?.minPrice)
            params.set(
                "minPrice",
                filters.filters.minPrice.toString()
            );
        if (filters.filters?.maxPrice)
            params.set(
                "maxPrice",
                filters.filters.maxPrice.toString()
            );
        if (filters.filters?.brand)
            params.set("brand", filters.filters.brand);
        if (filters.filters?.color)
            params.set("color", filters.filters.color);
        if (filters.filters?.size)
            params.set("size", filters.filters.size);
        if (filters.filters?.material)
            params.set(
                "material",
                filters.filters.material
            );
        if (filters.filters?.isFeatured)
    ...
    const handleFilterChange = (
        ...
        const isClearAll =
            newFilters.filters &&
            Object.keys(newFilters.filters).length === 0;
