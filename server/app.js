...
app.get("/products", (req, res) => {
  res.status(301).json({
    status: "redirect",
    message: "This endpoint has moved. Please use the API versioned endpoint.",
...
async function startServer() {
  try {
    ...
    server = app.listen(PORT, () => {
      ...
      logger.info(`REST API endpoint: http://localhost:${PORT}/api/v1/users`);
      logger.info(`REST API endpoint: http://localhost:${PORT}/api/v1/orders`);
    });
