...
class OrderController {
  ...
  async addOrderTransaction(req, res) {
    try {
      const order = await orderService.addOrderTransaction(
        req.body,
        req.user.id
      );
      res.json(order);
  ...
  async refundOrder(req, res) {
    try {
      const order = await orderService.refundOrder(req.body, req.user.id);
      res.json(order);
    } catch (error) {
      ...
      if (error.message.startsWith("Cannot refund order with payment status")) {
        return res.status(400).json({ error: error.message });
      }
