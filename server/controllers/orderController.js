...
class OrderController {
     ...
     * @param {Object} res - Express response object
     */
    async addOrderTransaction(req, res) {
        try {
            const order =
            ...
            console.error(
                "Error adding order transaction:",
                error
            );
    ...
    async addOrderTracking(req, res) {
        try {
            ...
            const order =
                await orderService.addOrderTracking(
                    req.params.id,
                    carrier,
                    trackingNumber,
                    estimatedDelivery
    ...
    async refundOrder(req, res) {
        try {
            ...
            if (
                error.message.startsWith(
                    "Cannot refund order with payment status"
                )
            ) {
                return res
                    .status(400)
    ...
    async getOrdersDashboard(req, res) {
        try {
            const dashboardData =
                await orderService.getOrdersDashboard();
            res.json(dashboardData);
        } catch (error) {
            console.error(
                "Error fetching orders dashboard:",
                error
            );
            res.status(500).json({ error: "Server error" });
        }
